...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
		require(totalLiquidity == 0, "zero token amount");

		liquidity[msg.sender] = msg.value;
		totalLiquidity = msg.value;

		require(token.transferFrom(msg.sender, address(this), tokens), "failed to transfer token in");
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
		...
		require(token.transfer(msg.sender, tokenOutput), "failed to transfer token out");
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
		require(tokenInput != 0, "zero tokenInput amount");

		uint256 tokenReserve = token.balanceOf(address(this));
		ethOutput = price(tokenInput, tokenReserve, address(this).balance);

		require(token.transferFrom(msg.sender, address(this), tokenInput), "failed to transfer token in");
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
		...
		require(token.transferFrom(msg.sender, address(this), tokensDeposited), "failed to transfer token in");
    ...
    function withdraw(uint256 amount) public returns (uint256 ethAmount, uint256 tokenAmount) {
		require(amount != 0, "zero liquidity amount");
		...
		require(token.transfer(msg.sender, tokenAmount), "failed to transfer token out");
