...
const deployVendor: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  // // Deploy Vendor //volani funkci ETH frameworku
  const { deployer } = await hre.getNamedAccounts();
  const { deploy } = hre.deployments;
  const yourToken = await hre.ethers.getContract<Contract>("YourToken", deployer); //najede definici kontraktu
  const yourTokenAddress = await yourToken.getAddress();
  await deploy("Vendor", {
    from: deployer,
    // Contract constructor arguments
    args: [yourTokenAddress], // z jake adresy to bereme
    log: true,
  ...
  await vendor.transferOwnership("0x472f45Ca19b7Cc1e4366c0d8bd8E61896d3c5048"); //moje adresa z FE jako owner?
};
